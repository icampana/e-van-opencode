...
function detectPackageManager(): string {
  const lockFiles = [
    { file: "bun.lock", cmd: "bunx tsc" },
    { file: "pnpm-lock.yaml", cmd: "pnpm exec tsc" },
    { file: "yarn.lock", cmd: "yarn exec tsc" },
    { file: "package-lock.json", cmd: "npm exec tsc" },
  ]

  for (const { file, cmd } of lockFiles) {
    if (Bun.file(file).exists()) return cmd
  }
...
export default tool({
  ...
  async execute({ file }) {
    try {
      const baseCmd = detectPackageManager()
      const cmd = file
        ? `${baseCmd} --noEmit "${file}" 2>&1`
        : `${baseCmd} --noEmit 2>&1`
