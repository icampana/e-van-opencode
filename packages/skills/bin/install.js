 ...
 *
 * Copies skill directories into the target project's .opencode/skills/ folder.
...
function printHelp() {
  console.log(`
${colorize('bold', '@e-van/skills')} — Install workflow skills for OpenCode + Superpowers
...
function parseArgs(argv) {
  const args = { force: false, list: false, coreOnly: false, path: null };
...
const targetSkillsDir = path.join(targetRoot, '.opencode', 'skills');
...
if (installed > 0) {
  ...
  console.log(colorize('gray', 'Restart your opencode session to load the new skills.'));
  console.log();
